...
if (isset($_POST['updateAccount'])) {
    ...
    if ($fname == $row['firstName'] && $lname == $row['lastName'] && $gender == $row['gender'] && $address == $row['address'] && $email == $row['email']) {
        $_SESSION['status'] = "fail";
        $_SESSION['message'] = 'Account not Updated!';
    } else {
        $stmt = $con->prepare("UPDATE `user` SET `firstName` = ?, `lastName` = ?, `gender` = ?, `address` = ?, `email` = ? WHERE `ID` = $accountId");
        $stmt->bind_param("sssss", $fname, $lname, $gender, $address, $email);
